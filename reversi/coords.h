...
struct coords {
	...
	coords() : coords(INCORRECT_POSITION, NONE) {}
	coords(coord x, coord y, figure piece = NONE) : x(x), y(y), piece(piece) {}
	std::string to_string() const { return char('a' + x) + std::to_string(y + 1) + (piece == BLACK ? "b" : piece == WHITE ? "w" : ""); }
